...
export default function Testimonials() {
  return (
    <section id="testimonios" className="py-20 bg-white border-t border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map((t, idx) => (
            <div key={idx} className="bg-slate-50 p-8 rounded-3xl border border-slate-200 flex flex-col justify-between space-y-4 hover:border-brand-400 transition shadow-sm">
              ...
              <div className="pt-4 border-t border-slate-200/80">
                ...
                <div className="text-[11px] text-brand-600 font-semibold">{t.zone}</div>
                <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-1">
                  <CheckCircle2 className="w-3 h-3 text-emerald-500" /> {t.equipment}
                </div>
              </div>
            </div>
          ))}
